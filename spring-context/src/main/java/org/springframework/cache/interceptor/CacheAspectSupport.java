 ...
 * <p>
 * *用于缓存方面的基类，例如{@link CacheInterceptor}或
 * * AspectJ方面。
 * *
 * * <p>这使得底层Spring缓存基础设施能够轻松使用
 * *为任何方面系统实现方面。
 * *
 * * <p>子类负责以正确的顺序调用相关的方法。
 * *
 * * <p>采用<b>策略</b>设计模式。一个{@link CacheOperationSource}是
 * *用于确定缓存操作，{@link KeyGenerator}将构建
 * *缓存键和{@link CacheResolver}将解析要使用的实际缓存。
 * *
 * 注意:缓存方面是可序列化的，但不执行任何实际的缓存
 * 反序列化后*。
 *
 * @author Costin Leau
