...
/**
 * 简单{@link org.springframework.cache。的缓存}实现
 * 核心JDK {@code java.util。并发}包。
 *
 * <p>用于测试或简单的缓存场景，通常结合使用
 * 具有{@link org.springframework.cache.support。SimpleCacheManager}或
 * 动态通过{@link ConcurrentMapCacheManager}。
 * <p>
 * 注:</b> As {@link ConcurrentHashMap}(使用默认实现)
 * 不允许存储{@code null}值，该类将替换
 * 它们带有预定义的内部对象。这种行为可以通过
 * {@link #ConcurrentMapCache(String, ConcurrentMap, boolean)}构造函数。
 * <p>
 * 作者Costin Leau
 * 作者Juergen Hoeller
 * 作者Stephane Nicoll
